...
static Window *window = NULL, *second_window = NULL;
...
// Delete the selected todo list item ATTENTION DOIT ETRE TRANSFORME EN CHECKBOX
...
void loading_window_load(Window *second_window)
{
	...
	layer_add_child(window_get_root_layer(second_window), text_layer_get_layer(loading_text_layer));
...
void get_list_items(int32_t item_id, char *item_title, Window *second_window, int32_t end_of_list)
{
	...
	APP_LOG(APP_LOG_LEVEL_INFO, "av list_item_add");
	list_items_add(item_title, item_id);
	APP_LOG(APP_LOG_LEVEL_INFO, "ap list item add");
	//menu_layer_reload_data(layer_list_items);

	// configure second_window
	if (end_of_list == 1)
	{
		APP_LOG(APP_LOG_LEVEL_INFO, "enter in creation de second window");
		...
		window_stack_push(second_window, NULL); // Animated
		loading = 0;
		APP_LOG(APP_LOG_LEVEL_INFO, "quit in creation de second window");
	}
	else if (loading == 0)
	{
		APP_LOG(APP_LOG_LEVEL_INFO, "enter in creation de loading window");
		// create and configure the second_window with a loading text layer
		second_window = window_create();
		window_set_window_handlers(second_window, (WindowHandlers) {
				.load = loading_window_load,
				});
		window_stack_push(second_window, true); // Animated
		loading = 1;
		APP_LOG(APP_LOG_LEVEL_INFO, "enter in creation de loading window");
	}
...
static void select_click_callback(MenuLayer *menu_layer, MenuIndex *cell_index, void *data)
{
	...
	if (layer_list_items)
	{
		APP_LOG(APP_LOG_LEVEL_INFO, "dans win_reg 2");
		window_destroy(second_window);
		second_window = NULL;

		menu_layer_destroy(layer_list_items);
		layer_list_items = NULL;
		APP_LOG(APP_LOG_LEVEL_INFO, "dans win_reg 2bis");
	}
	APP_LOG(APP_LOG_LEVEL_INFO, "dans win_reg 4");
	delete_list_items_content();

	APP_LOG(APP_LOG_LEVEL_INFO, "dans win_reg 5");

	/* listTitle taken for the futur header of the list_items MenuLayer*/
	listTitle = (char *)malloc(sizeof(char) * (strlen(item->text) + 1));
	if (listTitle != NULL) // a garder pour pouvoir sortir correctement du programme si le malloc ne se fait pas
	{
		APP_LOG(APP_LOG_LEVEL_INFO, "dans if listTitle 1");
		strcpy(listTitle, item->text);
		dict_write_end(iter);
		app_message_outbox_send();
		APP_LOG(APP_LOG_LEVEL_INFO, "dans if listTitle 2");
	}
	else
		return;
...
static void in_received_handler(DictionaryIterator *iter, void *context) 
{
	...
	switch ((MsgTypes)(msgid->value->int32))
	{
		case MENU_ITEM_TEXT:
			...
			if (end_of_list == 1)
			{
				//reload the menu of lists once all data needed has been transmitted
